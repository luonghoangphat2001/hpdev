...
class TimeUtils {
  ...
  /**
   * Add N minutes to a DB datetime string "YYYY-MM-DD HH:MM:SS".
   * Uses UTC arithmetic to avoid DST issues.
   * @param {string} dbStr
   * @param {number} minutes
   * @returns {string}  "YYYY-MM-DD HH:MM:SS"
   */
  static addMinutes(dbStr, minutes) {
    const s    = String(dbStr);
    const date = s.slice(0, 10);
    const time = s.slice(11) || '00:00:00';
    const [yyyy, mm, dd] = date.split('-').map(Number);
    const [hh, mi, ss]   = time.split(':').map(Number);
    const ms = Date.UTC(yyyy, mm - 1, dd, hh, mi + minutes, ss || 0);
    const d  = new Date(ms);
