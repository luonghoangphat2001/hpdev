<template>
    <div class="h-full flex flex-col min-w-0">
        ...
        <header class="h-14 px-4 border-b border-gray-800 bg-gray-900/90 backdrop-blur-md flex items-center justify-between shrink-0 overflow-x-auto">
            ...
            <!-- Action Buttons -->
            <button @click="showGenerator = true" class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-xs font-semibold text-white shadow-md shadow-indigo-500/20 hover:opacity-90 transition">
                <span>✨</span>
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-6 min-w-0">
            ...
            <div v-if="currentTab === 'tech'" class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                    <button v-for="s in learningStore.techStacks" :key="s.slug" @click="learningStore.setTechSlug(s.slug)" :class="['p-3.5 rounded-2xl border text-center transition flex flex-col items-center justify-center shadow-md', learningStore.activeTechSlug === s.slug ? 'bg-indigo-600/30 border-indigo-500 ring-2 ring-indigo-500/40 text-white' : 'bg-gray-800/80 border-gray-700/80 hover:bg-gray-700/80 text-gray-300']">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 bg-gray-800/70 p-3 rounded-2xl border border-gray-700/60">
                    <div class="flex items-center gap-2 flex-1 min-w-[200px]">
                        <input v-model="learningStore.searchQuery" @input="learningStore.loadQuestions" type="text" placeholder="Tìm kiếm câu hỏi..." class="w-full px-3 py-1.5 rounded-xl bg-gray-900 border border-gray-700 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500" />
                    </div>

                    <div class="flex items-center gap-2">
                        <select v-model="learningStore.filterLevel" @change="learningStore.loadQuestions" class="px-2.5 py-1.5 rounded-xl bg-gray-900 border border-gray-700 text-xs text-gray-300 focus:outline-none">
                        ...
                        <button @click="toggleBookmarkFilter" :class="['px-3 py-1.5 rounded-xl text-xs font-semibold border transition flex items-center gap-1', learningStore.filterBookmark ? 'bg-amber-500/20 border-amber-500/50 text-amber-300' : 'bg-gray-900 border-gray-700 text-gray-400']">
                            <span>★</span>
                            <span>Đã lưu</span>
                        </button>
                        ...
                        <div class="flex rounded-xl bg-gray-900 p-0.5 border border-gray-700">
                            <button @click="learningStore.viewMode = 'split'" :class="learningStore.viewMode === 'split' ? 'bg-indigo-600 text-white' : 'text-gray-400'" class="px-2.5 py-1 rounded-lg text-xs font-semibold transition">Split View</button>
                            <button @click="learningStore.viewMode = 'flashcard'" :class="learningStore.viewMode === 'flashcard' ? 'bg-indigo-600 text-white' : 'text-gray-400'" class="px-2.5 py-1 rounded-lg text-xs font-semibold transition">Flashcard 3D</button>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="learningStore.viewMode === 'split'" class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div class="md:col-span-5 space-y-2 max-h-[600px] overflow-y-auto pr-1">
                        <div v-for="(q, idx) in learningStore.techQuestions" :key="q.id" @click="learningStore.selectQuestion(q)" :class="['p-3.5 rounded-2xl border cursor-pointer transition flex items-start justify-between gap-2 shadow-sm', learningStore.activeQuestion?.id === q.id ? 'bg-indigo-600/20 border-indigo-500 text-white ring-1 ring-indigo-500/40' : 'bg-gray-800/80 border-gray-700/80 hover:bg-gray-700/60 text-gray-300']">
                            ...
                            <button @click.stop="learningStore.toggleBookmark(q)" class="text-gray-400 hover:text-amber-400 text-sm" :class="q.is_bookmarked ? 'text-amber-400' : ''">★</button>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-7 bg-gray-800/90 border border-gray-700/80 rounded-3xl p-6 shadow-xl flex flex-col justify-between">
                        ...
                        <div v-else class="h-64 flex items-center justify-center text-gray-500 text-sm">Chọn một câu hỏi ở danh sách bên trái để xem giải thích.</div>
                    </div>
                </div>
                ...
                <div v-else class="py-4">
                    <div v-if="learningStore.techQuestions.length > 0">
                        <Flashcard3D :question="learningStore.techQuestions[learningStore.flashcardIndex]" :index="learningStore.flashcardIndex" @bookmark="learningStore.toggleBookmark" />
                        <div class="flex items-center justify-center gap-4 mt-6">
                            <button @click="prevFlashcard" :disabled="learningStore.flashcardIndex === 0" class="px-4 py-2 rounded-xl bg-gray-800 border border-gray-700 hover:bg-gray-700 text-xs font-semibold disabled:opacity-40">← Câu trước</button>
                            <span class="text-xs font-mono text-gray-400"> {{ learningStore.flashcardIndex + 1 }} / {{ learningStore.techQuestions.length }} </span>
                            <button @click="nextFlashcard" :disabled="learningStore.flashcardIndex === learningStore.techQuestions.length - 1" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-xs font-semibold text-white disabled:opacity-40">Câu tiếp theo →</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="currentTab === 'quiz'">
                <QuizMode :questions="learningStore.techQuestions" @restart="learningStore.loadQuestions" />
            </div>
            ...
            <div v-else-if="currentTab === 'vocab'" class="max-w-4xl mx-auto space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div v-for="item in vocabList" :key="item.word" class="bg-gray-800/90 border border-gray-700/80 rounded-2xl p-5 shadow-lg">
            ...
            <div v-else-if="currentTab === 'writing'" class="max-w-4xl mx-auto bg-gray-800/90 border border-gray-700/80 rounded-3xl p-6 shadow-xl">
                ...
                <textarea v-model="writingText" rows="10" placeholder="Type your essay here..." class="w-full p-4 rounded-2xl bg-gray-900 border border-gray-700 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 mb-4"></textarea>

                <div class="flex items-center justify-between">
                    <span class="text-xs text-gray-400 font-mono">{{ wordCount }} từ</span>
                    <button @click="submitWriting" :disabled="!writingText.trim() || evaluatingWriting" class="px-5 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold transition disabled:opacity-50">
            ...
            <div v-else-if="currentTab === 'speaking'" class="max-w-2xl mx-auto bg-gray-800/90 border border-gray-700/80 rounded-3xl p-8 text-center shadow-xl">
                ...
                <div class="p-6 rounded-3xl bg-gray-900 border border-gray-700 mb-6 flex flex-col items-center">
                    <button @click="toggleRecording" :class="['w-20 h-20 rounded-full flex items-center justify-center text-2xl transition-all shadow-xl', isRecording ? 'bg-rose-600 text-white animate-pulse ring-4 ring-rose-500/40' : 'bg-indigo-600 text-white hover:bg-indigo-500']">
                        {{ isRecording ? "⏹" : "🎤" }}
                    </button>
        ...
        <!-- AI Generator Modal -->
        <AIGeneratorModal :is-open="showGenerator" @close="showGenerator = false" @generated="handleGenerated" />
    </div>
</template>
