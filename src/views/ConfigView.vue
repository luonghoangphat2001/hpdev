<template>
    <div class="h-full flex flex-col min-w-0">
        <header class="h-14 px-4 border-b border-gray-800 bg-gray-900/90 flex items-center justify-between shrink-0">
            ...
            <button @click="save" :disabled="configStore.saving" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold flex items-center gap-2 transition disabled:opacity-50">
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-6 max-w-5xl mx-auto w-full space-y-6">
            ...
            <div class="bg-gray-800/90 border border-gray-700/80 rounded-3xl p-6 shadow-xl">
                ...
                <textarea v-model="systemPrompt" rows="6" class="w-full p-4 rounded-2xl bg-gray-900 border border-gray-700 text-xs font-mono text-gray-200 focus:outline-none focus:border-indigo-500"></textarea>
            </div>
        </div>
    </div>
</template>
