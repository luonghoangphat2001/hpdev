    ...
    <div class="max-w-7xl mx-auto px-3.5 sm:px-6 py-4 sm:py-8 space-y-4 sm:space-y-6 safe-area-pb">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 pb-4 sm:pb-6 border-b border-gray-800">
        ...
        <div class="flex items-center gap-2 sm:gap-3 self-end sm:self-auto">
          <span v-if="savedMsg" class="text-xs text-emerald-400 font-medium px-3 py-1.5 rounded-xl bg-emerald-950/60 border border-emerald-800/60 flex items-center gap-1.5 shadow-sm">
            <span>✓</span> Đã lưu!
          </span>
          <button 
            @click="saveConfig" 
            :disabled="saving"
            class="px-4 sm:px-5 py-2 sm:py-2.5 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white rounded-xl font-semibold text-xs transition shadow-lg shadow-indigo-600/30 flex items-center gap-1.5 sm:gap-2 active:scale-95 disabled:opacity-50"
          >
            <span>💾</span> {{ saving ? 'Đang lưu...' : 'Lưu cấu hình' }}
          </button>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 border-b border-gray-800 pb-2 overflow-x-auto">
        <router-link 
          ...
          :to="'/config/' + t.id"
          :class="[
            'px-3.5 py-2 rounded-xl text-xs font-semibold transition flex items-center gap-2 whitespace-nowrap',
            currentTab === t.id ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-800 hover:text-white'
          ]"
        >
          <span>{{ t.icon }}</span>
          <span>{{ t.label }}</span>
      ...
      <div class="pt-1 sm:pt-2">
        ...
        <div v-show="currentTab === 'providers'" class="space-y-6">
          <div class="grid md:grid-cols-2 gap-6">
            <div class="bg-gray-800/90 rounded-xl p-6 border border-gray-700/60 shadow-lg">
              <h2 class="font-semibold mb-1 text-gray-200 flex items-center gap-2">
                <span>🌟</span> Gemini Model
              </h2>
          ...
          <div class="bg-gray-800/90 rounded-xl p-6 border border-gray-700/60 shadow-lg">
            <h2 class="font-semibold mb-1 text-gray-200 flex items-center gap-2">
              <span>✳️</span> Claude Model & Base URL
            </h2>
        ...
        <div v-show="currentTab === 'openclaw'" class="space-y-6">
          <div class="bg-gray-800/90 rounded-xl p-6 border border-gray-700/60 shadow-lg space-y-4">
            <h2 class="font-semibold text-gray-200 flex items-center gap-2">
              <span>🦅</span> OpenClaw Autonomous Agents
            </h2>
        ...
        <div v-show="currentTab === 'prompts'" class="space-y-6">
          ...
          <div class="bg-gray-800/90 rounded-xl p-6 border border-gray-700/60 shadow-lg">
            <h2 class="font-semibold mb-1 text-gray-200 flex items-center gap-2">
              <span>📝</span> System Persona Prompt
            </h2>
          ...
          <div class="bg-gray-800/90 rounded-xl p-6 border border-gray-700/60 shadow-lg space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h2 class="font-semibold mb-1 text-gray-200 flex items-center gap-2">
                  <span>🎓</span> Learning Hub — Tuỳ biến System Prompts AI
                </h2>
                <p class="text-xs text-gray-400" v-pre>Tuỳ chỉnh System Prompt cho các chức năng sinh bài & chấm điểm (để trống nếu muốn dùng mặc định). Hỗ trợ biến: <code class="text-indigo-300">{{stackName}}</code>, <code class="text-indigo-300">{{topicName}}</code>, <code class="text-indigo-300">{{level}}</code>, <code class="text-indigo-300">{{count}}</code>, <code class="text-indigo-300">{{existingWords}}</code></p>
              </div>
              <button 
                @click="fillAllPromptTemplates" 
                type="button" 
                class="px-3 py-1.5 bg-indigo-900/60 hover:bg-indigo-800 text-indigo-300 rounded-lg text-xs font-medium border border-indigo-700 transition flex items-center gap-1.5 whitespace-nowrap self-start sm:self-auto"
              >
                📋 Nhập tất cả mẫu
              </button>
            </div>
        ...
        <div v-show="currentTab === 'logs'" class="space-y-6">
          <div class="bg-gray-800/90 rounded-xl p-6 border border-gray-700/60 shadow-lg space-y-4">
            <h2 class="font-semibold text-gray-200 flex items-center gap-2">
              <span>🪵</span> Cấu hình Nhật ký Logs & Lưu trữ
            </h2>
      ...
      <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-4 pb-8 border-t border-gray-800">
        ...
        <button 
          @click="saveConfig" 
          :disabled="saving"
          class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white rounded-xl font-semibold text-xs transition shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50"
        >
          <span>💾</span> {{ saving ? 'Đang lưu...' : 'Lưu toàn bộ cấu hình' }}
        </button>
      </div>
