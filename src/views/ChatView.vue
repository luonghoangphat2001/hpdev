<template>
    <div id="page-chat" class="flex flex-col h-full min-h-0 overflow-hidden bg-gray-900 text-gray-100">
        ...
        <div ref="chatContainer" id="chat-window" class="flex-1 overflow-y-auto py-4 sm:py-6 px-3 sm:px-6 space-y-4 sm:space-y-6 touch-scroll">
            ...
            <!-- Message Bubbles -->
            <div v-for="msg in chatStore.messages" :key="msg.id" :class="['flex gap-2.5 sm:gap-3 max-w-3xl w-full', msg.role === 'user' ? 'ml-auto flex-row-reverse' : 'mr-auto']">
                <div class="shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow" :class="msg.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-indigo-400 border border-gray-700'">
                    {{ msg.role === "user" ? authStore.userInitial : "🤖" }}
                </div>

                <div :class="['p-3.5 sm:p-4 rounded-2xl text-sm leading-relaxed max-w-[85%] shadow whitespace-pre-wrap break-words', msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : msg.isError ? 'bg-red-900/30 border border-red-700/60 text-red-300' : 'bg-gray-800 border border-gray-700 text-gray-200 rounded-tl-none']">
                    <p>{{ msg.content }}</p>
        ...
        <div class="shrink-0 border-t border-gray-700/80 bg-gray-900 px-3 sm:px-4 py-2.5 sm:py-4">
            <div class="max-w-3xl mx-auto">
                <form @submit.prevent="handleSend" class="flex items-end gap-2 sm:gap-3 bg-gray-800 rounded-2xl border border-gray-700 focus-within:border-indigo-500 px-3 sm:px-4 py-2.5 sm:py-3 transition shadow-lg">
                    <textarea v-model="inputText" @keydown.enter.exact.prevent="handleSend" rows="1" placeholder="Hỏi Đần điều gì đó… (Shift+Enter để xuống dòng)" class="flex-1 bg-transparent resize-none text-base sm:text-sm focus:outline-none text-white placeholder-gray-500 max-h-36 sm:max-h-40 leading-relaxed"></textarea>
                    <button type="submit" :disabled="!inputText.trim() || chatStore.loading" class="shrink-0 w-9 h-9 sm:w-8 sm:h-8 flex items-center justify-center bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white rounded-xl sm:rounded-lg transition disabled:opacity-40 disabled:cursor-not-allowed active:scale-95 shadow" aria-label="Gửi tin nhắn">
