  ...
  <div class="h-full flex flex-col min-w-0">
    ...
    <header class="h-14 px-4 border-b border-gray-800 bg-gray-900/80 backdrop-blur-md flex items-center justify-between shrink-0">
      ...
      <button 
        ...
        class="text-xs text-gray-400 hover:text-gray-200 px-2.5 py-1 rounded-lg hover:bg-gray-800 transition"
      >
        Làm mới chat
      </button>
    </header>
    ...
    <div ref="chatContainer" class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 max-w-4xl w-full mx-auto">
      ...
      >
        <div 
          class="shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-md"
        ...
        <div 
          :class="[
            'p-4 rounded-3xl text-sm leading-relaxed max-w-[85%] shadow-md whitespace-pre-wrap break-words',
            msg.role === 'user' 
              ? 'bg-indigo-600 text-white rounded-tr-none' 
              : (msg.isError ? 'bg-rose-500/20 border border-rose-500/30 text-rose-300' : 'bg-gray-800/90 border border-gray-700/80 text-gray-200 rounded-tl-none')
          ]"
        >
          <p>{{ msg.content }}</p>
      ...
      <div v-if="chatStore.loading" class="flex items-center gap-3 mr-auto">
        <div class="w-8 h-8 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center text-xs text-indigo-400">
          🤖
        </div>
      ...
      <form @submit.prevent="handleSend" class="max-w-4xl mx-auto flex items-end gap-2">
        <div class="flex-1 bg-gray-800/90 border border-gray-700 rounded-2xl overflow-hidden focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 transition">
          <textarea 
            ...
            rows="1"
            placeholder="Hỏi bất cứ điều gì... (Enter để gửi, Shift+Enter xuống dòng)"
            class="w-full px-4 py-3 bg-transparent text-sm text-white placeholder-gray-500 focus:outline-none resize-none max-h-32 min-h-[44px]"
          ></textarea>
        </div>
        ...
        <button 
          ...
          class="h-[44px] px-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white font-bold text-sm transition shadow-lg shadow-indigo-600/30 disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center"
        >
