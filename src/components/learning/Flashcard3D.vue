<template>
    <div class="w-full max-w-2xl mx-auto perspective-1000 my-4">
        <div @click="flipped = !flipped" :class="['relative w-full min-h-[320px] rounded-3xl p-6 cursor-pointer transition-all duration-500 transform-style-3d shadow-2xl border', flipped ? 'rotate-y-180 bg-gray-800/95 border-indigo-500/50 ring-2 ring-indigo-500/30' : 'bg-gray-800/90 border-gray-700/80 hover:border-gray-600']">
            <!-- Front Side (Question) -->
            <div v-if="!flipped" class="flex flex-col h-full justify-between backface-hidden">
                <div>
                    <div class="flex items-center justify-between gap-2 mb-4">
                        ...
                        <button @click.stop="$emit('bookmark', question)" class="p-1.5 rounded-lg text-gray-400 hover:text-amber-400 transition" :class="question?.is_bookmarked ? 'text-amber-400' : ''" title="Bookmark">★</button>
                    </div>
