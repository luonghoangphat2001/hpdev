  ...
  <div class="max-w-2xl mx-auto p-4">
    ...
    <div class="flex items-center justify-between bg-gray-800/90 border border-gray-700/80 rounded-2xl p-4 mb-4">
      ...
      <div class="flex items-center gap-2">
        <div 
          class="w-10 h-10 rounded-full flex items-center justify-center font-mono font-bold text-sm border-2 transition"
          :class="timeLeft <= 5 ? 'border-rose-500 text-rose-400 bg-rose-500/10 animate-pulse' : 'border-indigo-500 text-indigo-400 bg-indigo-500/10'"
        >
          {{ timeLeft }}
        </div>
      </div>
    ...
    <div v-if="currentQuestion" class="bg-gray-800/90 border border-gray-700/80 rounded-3xl p-6 shadow-xl mb-4">
      ...
      <div class="space-y-3">
        <button 
          ...
          :disabled="answered"
          :class="[
            'w-full text-left p-4 rounded-2xl border text-sm font-medium transition flex items-center justify-between',
            getOptionClass(opt, idx)
          ]"
        >
          <span>{{ opt }}</span>
    ...
    <div v-else class="bg-gray-800/90 border border-gray-700/80 rounded-3xl p-8 text-center shadow-xl">
      ...
      <p class="text-sm text-gray-300 mb-6">Bạn đã đạt được <strong class="text-indigo-400 font-bold">{{ score }}</strong> điểm với chuỗi tối đa <strong class="text-amber-400">{{ maxStreak }}</strong> câu!</p>
      <button 
        @click="$emit('restart')"
        class="px-6 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold transition shadow-lg shadow-indigo-600/30"
      >
        Làm lại bài mới
      </button>
    </div>
...
const options = computed(() => {
  ...
  return [
    currentQuestion.value.answer || 'Đáp án chính xác',
    'Lựa chọn A thay thế',
    'Lựa chọn B thay thế',
    'Lựa chọn C thay thế',
  ];
