...
const actionCatalog = require('./ecommerce-action.catalog');
...
function schema(actionName, properties = {}, required = []) {
  return Object.freeze({
    ...
    $id: `https://openclaw.hpdev.name.vn/schemas/v1/actions/${actionName}.json`,
...
const ACTION_REQUEST_SCHEMAS = Object.freeze({
  'order.list': schema('order.list', {
    ...
    status: {
      type: 'string',
      enum: ['pending', 'new', 'processing', 'delivering', 'completed', 'cancelled', 'refunded'],
    },
  }),
