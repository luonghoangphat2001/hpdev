      ...
      <form @submit.prevent="generate" class="space-y-4">
        <div>
          ...
          <input 
            v-model="topic" 
            type="text" 
            placeholder="vd: React Hooks, Docker Networking, IELTS Speaking Part 2" 
            required 
            class="w-full px-3.5 py-2.5 rounded-xl bg-gray-900 border border-gray-700 text-sm text-white focus:outline-none focus:border-indigo-500"
          />
        </div>
        ...
        <div class="pt-3 flex items-center justify-end gap-2">
          <button 
            type="button" 
            @click="$emit('close')" 
            class="px-4 py-2 rounded-xl text-xs font-semibold text-gray-400 hover:text-white transition"
          >
            Hủy
          </button>
          <button 
            type="submit" 
            :disabled="loading" 
            class="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold flex items-center gap-2 transition disabled:opacity-60"
          >
