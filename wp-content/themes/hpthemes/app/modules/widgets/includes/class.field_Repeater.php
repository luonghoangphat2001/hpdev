...
class Render_Field_Repeater {
    ...
    public function render( $field_args = [] ) 
    {
        ...
        if ( isset ( $field_args['title'] ) ) {
            ...
            $widget_title = __( 'Repeater', 'HPmedia' );
        }
        ...
        $output .= '<div class="hp-widget-items render-field box-field-repeater repeaters">';
        ...
        $output .= '<span>'.__( 'rowCloneindex', 'HPmedia' ).'</span>';
        $output .= '</div>';
        ...
        foreach ( $field_args['fields'] as $key => $item ) {
            ...
            if ( $item['type'] == 'text' ) {
                ...
                if ( isset ( $item['title'] ) ) {
                    ...
                    $item_title = __( 'Văn bản ngắn', 'HPmedia' );
                }

                if ( isset ( $item['class'] ) ) {
                    $class = 'class="hp-custom-widget ref-field-text '.esc_attr( $item['class'] ).'"';
                } else {
                ...
                if ( isset ( $item['title'] ) ) {
                    ...
                    $item_title = __( 'Văn bản dài', 'HPmedia' );
                }

                if ( isset ( $item['class'] ) ) {
                    $class = 'class="hp-custom-widget ref-field-textarea '.esc_attr( $item['class'] ).'"';
                } else {
                ...
                if ( isset ( $item['title'] ) ) {
                    ...
                    $item_title = __( 'Hình ảnh', 'HPmedia' );
                }

                if ( isset ( $item['class'] ) ) {
                    $class = 'class="hp-custom-widget ref-field-image '.esc_attr( $item['class'] ).'"';
                } else {
                    $class = 'class="hp-custom-widget ref-field-image"';
                }
                ...
                if ( isset( $item['default'] ) && ! empty ( $item['default'] ) AND empty ( $image_src ) ) {
                    $default_src = esc_url( $item['default'] );
                }
                ...
                $output .= '<img default-src="'.$default_src.'" style="width:auto; height:auto; max-width:'.$width.'px;" src="'.$image_src.'" class="w-image-review"/>';
                $output .= '<div class="box-image-button">';
                $output .= '<button class="upload_image_button button button-primary">'.__( 'Chọn ảnh', 'HPmedia' ).'</button>';
                if ( ! empty ( $field_args['value'][$key] ) ) {
                    $output .= '<button class="remove_image_button button button-danger">'.__( 'Xóa ảnh', 'HPmedia' ).'</button>';
                }
                $output .= '</div>';
                $output .= '</div>';
                $output .= '</div>';
                ...
                if ( isset ( $item['title'] ) ) {
                    ...
                    $item_title = __( 'Select', 'HPmedia' );
                }

                if ( isset ( $item['class'] ) ) {
                    $class = 'class="hp-custom-widget ref-field-select '.esc_attr( $item['class'] ).'"';
                } else {
                ...
                if ( isset ( $item['title'] ) ) {
                    ...
                    $item_title = __( 'Radio', 'HPmedia' );
                }

                if ( isset ( $item['class'] ) ) {
                    $class = 'class="hp-custom-widget ref-field-radio '.esc_attr( $item['class'] ).'"';
                } else {
                ...
                if ( isset ( $item['title'] ) ) {
                    ...
                    $item_title = __( 'Checkbox', 'HPmedia' );
                }

                if ( isset ( $item['class'] ) ) {
                    $class = 'class="hp-custom-widget ref-field-checkbox '.esc_attr( $item['class'] ).'"';
                } else {
                    $class = 'class="hp-custom-widget ref-field-checkbox"';
                }
                ...
                if ( isset ( $item['title'] ) ) {
                    ...
                    $item_title = __( 'Gallery', 'HPmedia' );
                }

                if ( isset ( $item['class'] ) ) {
                    $gallery_class = 'class="hp-custom-widget ref-field-gallery '.esc_attr( $item['class'] ).'"';
                } else {
                    $gallery_class = 'class="hp-custom-widget ref-field-gallery"';
                }
                ...
                $output .= '<button class="upload_gallery_button button button-primary">'.__( 'Chọn ảnh', 'HPmedia' ).'</button>';
                $output .= '</div>';
                $output .= '</div>';
                $output .= '<script>
                ...
                if ( isset ( $item['title'] ) ) {
                    ...
                    $item_title = __( 'True / False', 'HPmedia' );
                }

                if ( isset ( $item['class'] ) ) {
                    $class = 'class="hp-custom-widget ref-field-truefalse hp-checkbox-field '.esc_attr( $item['class'] ).'"';
                } else {
                    $class = 'class="hp-custom-widget ref-field-truefalse hp-checkbox-field"';
                }

                $output .= '<div class="box-field-item render-item box-field-truefalse">';
                $output .= '<div class="box-field-title">';
                $output .= '<label '.$for.' class="txt-label field-text-label">';
                $output .=  esc_attr( $item_title );
                $output .= '</label>';
        ...
        $output .= '<button class="remove_row_button button button-danger">'.__( 'Xóa dòng', 'HPmedia' ).'</button>';
        $output .= '</div>';
        $output .= '</div>';
        $output .= '</div>';
        /**
                    ...
                    foreach ( $field_args['fields'] as $k => $field ) {
                        ...
                        if ( $field_args['fields'][$k]['type'] == 'text' ) {
                            ...
                            if ( isset ( $field_args['fields'][$k]['class'] ) ) {
                                $class = 'class="hp-custom-widget ref-field-text '.$field_args['fields'][$k]['class'].'"';
                            } else {
                                $class = 'class="hp-custom-widget ref-field-text"';
                            }
                            ...
                            if ( isset ( $field_args['fields'][$k]['class'] ) ) {
                                $class = 'class="hp-custom-widget ref-field-textarea '.$field_args['fields'][$k]['class'].'"';
                            } else {
                                $class = 'class="hp-custom-widget ref-field-textarea"';
                            }
                            ...
                            if ( isset ( $field_args['fields'][$k]['class'] ) ) {
                                $class = 'class="hp-custom-widget ref-field-image '.$field_args['fields'][$k]['class'].'"';
                            } else {
                                $class = 'class="hp-custom-widget ref-field-image"';
                            }
                            ...
                            if ( isset( $field_args['fields'][$k]['default'] ) && ! empty ( $field_args['fields'][$k]['default'] ) AND empty ( $image_src ) ) {
                                $default_src = esc_url( $field_args['fields'][$k]['default'] );
                            ...
                            if ( isset ( $field_args['fields'][$k]['class'] ) ) {
                                $class = 'class="hp-custom-widget ref-field-select '.$field_args['fields'][$k]['class'].'"';
                            } else {
                                $class = 'class="hp-custom-widget ref-field-select"';
                            }
                            ...
                            if ( isset ( $field_args['fields'][$k]['class'] ) ) {
                                $class = 'class="hp-custom-widget ref-field-radio '.$field_args['fields'][$k]['class'].'"';
                            } else {
                                $class = 'class="hp-custom-widget ref-field-radio"';
                            }
                            ...
                            if ( isset ( $field_args['fields'][$k]['class'] ) ) {
                                $class = 'class="hp-custom-widget ref-field-checkbox '.$field_args['fields'][$k]['class'].'"';
                            } else {
                                $class = 'class="hp-custom-widget ref-field-checkbox"';
                            }
                            ...
                            if ( ! empty ( $field_args['fields'][$k]['type']['class'] ) ) {
                                $gallery_class = 'hp-custom-widget ref-field-gallery '.$field_args['fields'][$k]['type']['class'].'';
                            } else {
                                $gallery_class = 'hp-custom-widget ref-field-gallery';
                            }
                            ...
                            if ( ! empty ( $field_args['value'][$key][$k] ) ) {
                                ...
                                foreach ( $field_args['value'][$key][$k] as $gallery => $value ) {
                                    ...
                                    $output .= '<a href="javascript:;" class="act-remove-image">'.__( 'Xóa', 'HPmedia' ).'</a>';
                                    $output .= '</div>';
                                    $output .= '</div>';
                                    $output .= '</div>';
                                    $image_stt++;
                        ...
                        } elseif ( $field_args['fields'][$k]['type'] == 'truefalse' ) {
                            ...
                            if ( isset ( $field_args['fields'][$k]['class'] ) ) {
                                $class = 'class="hp-custom-widget ref-field-truefalse hp-checkbox-field '.$field_args['fields'][$k]['class'].'"';
                            } else {
                                $class = 'class="hp-custom-widget ref-field-truefalse hp-checkbox-field"';
                            }
                            ...
                            $output .= '<div class="box-field-content">';
                            if ( isset ( $field_args['id'] ) ) {
                                $for = $field_args['id'].'-row'.'-'.$row.'-'.$k;
                            } else {
                                $for = $field_args['name'].'-row'.'-'.$row.'-'.$k;
                            }

                            $output .= '<div class="field-truefalse-item" '.$style_clss.'>';
                            $output .= '<div class="truefalse-text hp--checkboxWrap">';
                            if ( ! empty ( $checked ) && sanitize_title( $checked ) == true ) {
                                $output .= '<input type="checkbox" '.$class.' id="'.$for.'" '.$name.' value="1" checked="checked" />';
                            } else {
                                $output .= '<input type="checkbox" '.$class.' id="'.$for.'" '.$name.' value="1" />';
                            }
                            $output .= '<label for="'.$for.'" class="hp-checkbox-label"></label>';
                            $output .= '</div>';
                            $output .= '</div>';
                            $output .= '</div>';
                            $output .= '</div>';
        ...
        $output .= '<button class="add_row_button button button-primary ali-bt">'.__( 'Thêm dòng', 'HPmedia' ).'</button>';
        if ( isset ( $row ) && $row >= 2 ) {
            $output .= '<a href="javascript:;" class="order_field_button button button-primary ali-bt">'.__( 'Sắp sếp', 'HPmedia' ).'</a>';
        }
        $output .= '<script>
        jQuery(document).ready(function($) {
    ...
    public function get_docs() 
    {
        ...
        <pre>
            ...
            // Cập nhật
            $instance['repeater'] = $this->Hp_Widget->update_field( $new_instance['repeater'] );
            </code>
        </pre>
        <?php
    }
