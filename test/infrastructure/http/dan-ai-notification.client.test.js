...
const DanAiNotificationClient = require('../../../src/infrastructure/http/dan-ai-notification.client');
const {
  DanAiNotificationClientError,
} = require('../../../src/infrastructure/http/dan-ai-notification.client');

describe('DanAiNotificationClient', () => {
  const config = {
  ...
  test('uses only dan_ai API credentials and returns the queue receipt', async () => {
    ...
    const client = new DanAiNotificationClient({ httpClient, config });
    const payload = {
  ...
  test('fails closed when the adapter API is not configured', () => {
    expect(() => new DanAiNotificationClient({
      httpClient: {},
